...
from utils import V, get_ic_dom, get_ic_ref
...
class CaseOneField(InverseIVP):
    ...
    def r_net(self, params, t, T):
        ...
        R1 = params['params']['R1'] * u2 # mult
        C1 = params['params']['C1']
        return u1_t + (1/(R1*C1))*(1-(V/u2)*jnp.exp(-u1))


    @partial(jit, static_argnums=(0,))
    def losses(self, params, batch):
        # Data loss
        u1_pred, _ = self.u_pred_fn(params, self.t_ref, self.T_ref)
        data_loss = jnp.mean((self.u1_ref - u1_pred) ** 2)

        # IC loss
        t0, T0 = get_ic_dom()
        _, u2_t0 = self.u_pred_fn(params, t0, T0)
        R1 = params['params']['R1'] * u2_t0 # mult
        ic = jnp.log(V/u2_t0 + V/R1)
    ...
    def compute_l2_error(self, params, u1_test, u2_test):
        u1_pred, _ = vmap(self.u_net, (None, 0, 0))(params, self.t_ref, self.T_ref)
