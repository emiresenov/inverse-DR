...
from utils import V, get_initial_values, get_ic


class CaseOneField(InverseIVP):
    def __init__(self, config, t_ref, T_ref, u1_ref, u2_ref, u3_ref):
    ...
    def u_net(self, params, t, T):
        z = jnp.stack([t, T])
        u1, u2, u3 = self.state.apply_fn(params, z)
        return u1[0], u2[0], u3[0]
    
    def u1_net(self, params, t, T):
        u1, _, _ = self.u_net(params, t, T)
        return u1
    
    def r_net(self, params, t, T):
        u1, u2, u3 = self.u_net(params, t, T)
        u1_t = grad(self.u1_net, argnums=1)(params, t, T)
        C1 = params['params']['C1']
        return u1_t + (u1 - V/u2)/(u3*C1)


    @partial(jit, static_argnums=(0,))
    def losses(self, params, batch):
        # Data loss
        u1_pred,_,_ = self.u_pred_fn(params, self.t_ref, self.T_ref)
        data_loss = jnp.mean((self.u1_ref - u1_pred) ** 2)

        # IC loss
        t0, T0 = get_initial_values()
        _, u2_t0, u3_t0 = self.u_pred_fn(params, t0, T0)
        ic = V/u2_t0 + V/u3_t0
        ics_loss = jnp.mean((self.ic_ref - ic) ** 2)
    ...
    def compute_l2_error(self, params, u1_test, u2_test):
        u1_pred, _, _ = vmap(self.u_net, (None, 0, 0))(params, self.t_ref, self.T_ref)
        error = jnp.linalg.norm(u1_pred - u1_test) / jnp.linalg.norm(u1_test)
        return error
...
class CaseOneFieldEvaluator(BaseEvaluator):
    ...
    def log_preds(self, params):
        u1_pred, u2_pred, u3_pred = vmap(self.model.u_net, (None, 0, 0))(params, self.model.t_ref, self.model.T_ref)
        ...
        # u3 plot in 2D
        fig = plt.figure(figsize=(6, 5))
        plt.scatter(self.model.T_ref, self.model.u3_ref, s=50, alpha=0.9, c='orange')
        plt.plot(self.model.T_ref, u3_pred, linewidth=8, c='black')
        self.log_dict["R1 subnet"] = fig
        plt.close()
    ...
    def __call__(self, state, batch, u1_ref, u2_ref, u3_ref):
